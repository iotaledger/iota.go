...
type AddressSigner interface {
	...
	// It optionally also returns a public key depending on the signing scheme.
	Sign(addr Serializable, msg []byte) (signature []byte, publicKey []byte, err error)
}

// AddressSignerFunc implements the AddressSigner interface.
type AddressSignerFunc func(addr Serializable, msg []byte) (signature []byte, publicKey []byte, err error)

func (s AddressSignerFunc) Sign(addr Serializable, msg []byte) (signature []byte, publicKey []byte, err error) {
	return s(addr, msg)
}
...
func (s *InMemoryAddressSigner) Sign(addr Serializable, msg []byte) ([]byte, []byte, error) {
	switch addr.(type) {
	case *WOTSAddress:
		// TODO: implement
	...
	case *Ed25519Address:
		...
		if !ok {
			return nil, nil, fmt.Errorf("%w: can't sign message for Ed25519 address", ErrAddressKeysMissing)
		}
		sig := ed25519.Sign(prvKey, msg)
		return sig, prvKey.Public().(ed25519.PublicKey), nil
	default:
		return nil, nil, fmt.Errorf("%w: unknown address type %T", ErrUnknownAddrType, addr)
	}
}
