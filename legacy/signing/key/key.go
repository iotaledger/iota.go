...
In order to use the Keccak sponge construction, we must use the SHAKE extendable-output function producing a digest of
...
func Shake(entropy Trits, securityLevel legacy.SecurityLevel) (Trits, error) {
	if err := validateEntropy(entropy); err != nil {
		return nil, err
	}
	// the last trit will always be ignored, make sure it is always zero
	if entropy[legacy.HashTrinarySize-1] != 0 {
		return nil, errors.Wrapf(legacy.ErrInvalidTrit, "%d at index %d (last trit non 0)", entropy[legacy.HashTrinarySize-1], legacy.HashTrinarySize-1)
	}
	...
	key := make(Trits, legacy.KeyFragmentLength*int(securityLevel))
	buf := make([]byte, legacy.HashBytesSize)
	for i := 0; i < len(key); i += legacy.HashTrinarySize {
...
func Sponge(entropy Trits, securityLevel legacy.SecurityLevel, spongeFunc sponge.SpongeFunction) (Trits, error) {
	if err := validateEntropy(entropy); err != nil {
		return nil, err
	}
	// Kerl ignores the last trit, make sure it is always zero
	if _, isKerl := spongeFunc.(*kerl.Kerl); isKerl && entropy[legacy.HashTrinarySize-1] != 0 {
		return nil, errors.Wrapf(legacy.ErrInvalidTrit, "%d at index %d (last trit non 0)", entropy[legacy.HashTrinarySize-1], legacy.HashTrinarySize-1)
	}
	...
	for i := 0; i < len(key); i += legacy.HashTrinarySize {
		...
		out, err := spongeFunc.Squeeze(legacy.HashTrinarySize)
		if err != nil {
			return nil, err
		}
		copy(key[i:], out)
...
func validateEntropy(entropy Trits) error {
	...
	if len(entropy) != legacy.HashTrinarySize {
		return errors.Wrapf(legacy.ErrInvalidTritsLength, "must be %d in size", legacy.HashTrinarySize)
	}
