...
var nullHashTrits = make(trinary.Trits, legacy.HashTrinarySize)

func ReadTestJSONFile(t *testing.T, name string, v interface{}) {
...
func TestShake(t *testing.T) {
	t.Run("null entropy", func(t *testing.T) {
		var (
			...
			shakeTrits trinary.Trits
		)

		shakeSum, _ := hex.DecodeString(shakeHex)
		for i := 0; i < len(shakeSum); i += legacy.HashBytesSize {
			trits, _ := kerl.KerlBytesToTrits(shakeSum[i : i+legacy.HashBytesSize])
			shakeTrits = append(shakeTrits, trits...)
		}

		tests := []struct {
			name   string
			secLvl legacy.SecurityLevel
		}{
			{name: "SecurityLevelLow", secLvl: legacy.SecurityLevelLow},
			{name: "SecurityLevelMedium", secLvl: legacy.SecurityLevelMedium},
			{name: "SecurityLevelHigh", secLvl: legacy.SecurityLevelHigh},
		}

		for _, test := range tests {
			t.Run(test.name, func(t *testing.T) {
				k, err := key.Shake(nullHashTrits, test.secLvl)
				assert.NoError(t, err)
				assert.Len(t, k, int(test.secLvl)*legacy.SignatureMessageFragmentTrinarySize)
				assert.Equal(t, k, shakeTrits[:len(k)])
			})
		}
	})

	t.Run("valid entropy", func(t *testing.T) {
		t.Run("testdata/shake.json", func(t *testing.T) {
			var tests []*struct {
				Entropy       trinary.Trytes       `json:"entropy"`
				SecurityLevel legacy.SecurityLevel `json:"securityLevel"`
				Key           trinary.Trytes       `json:"key"`
			}
			ReadTestJSONFile(t, "shake.json", &tests)

			for _, tt := range tests {
				t.Run(tt.Entropy, func(t *testing.T) {
					k, err := key.Shake(trinary.MustTrytesToTrits(tt.Entropy), tt.SecurityLevel)
					assert.NoError(t, err)
					assert.Equal(t, tt.Key, trinary.MustTritsToTrytes(k))
				})
			}
		})
	})

	t.Run("invalid entropy", func(t *testing.T) {
		t.Run("returns error for empty entropy", func(t *testing.T) {
			_, err := key.Shake(nil, legacy.SecurityLevelMedium)
			assert.Error(t, err)
		})

		t.Run("returns error for invalid trits ", func(t *testing.T) {
			trits := make(trinary.Trits, legacy.HashTrinarySize)
			trits[0] = legacy.MaxTritValue + 1
			_, err := key.Shake(trits, legacy.SecurityLevelMedium)
			assert.Error(t, err)
		})

		t.Run("returns error when last trit is non-zero", func(t *testing.T) {
			trits := make(trinary.Trits, legacy.HashTrinarySize)
			trits[legacy.HashTrinarySize-1] = 1
			_, err := key.Shake(trits, legacy.SecurityLevelMedium)
			assert.Error(t, err)
		})
	})
...
func ExampleShake() {
	...
	privKey, err := key.Shake(entropy, legacy.SecurityLevelMedium)
	if err != nil {
		// handle error
		return
	}
	fmt.Println(trinary.MustTritsToTrytes(privKey))
	// Output: 
}

func TestSponge(t *testing.T) {

	t.Run("Sponge()", func(t *testing.T) {

		t.Run("Kerl", func(t *testing.T) {
			...
			for _, test := range tests {
				t.Run(test.name, func(t *testing.T) {
					sponge := kerl.NewKerl()
					key, err := key.Sponge(nullHashTrits, test.secLvl, sponge)
					assert.NoError(t, err)
					assert.Len(t, trinary.MustTritsToTrytes(key), int(test.secLvl)*legacy.SignatureMessageFragmentSizeInTrytes)
					assert.Equal(t, trinary.MustTritsToTrytes(key), kerlTrytes[:len(key)/legacy.TritsPerTryte])
				})
			}
		})
		...
		t.Run("invalid entropy", func(t *testing.T) {
			t.Run("returns error for empty entropy", func(t *testing.T) {
				_, err := key.Sponge(nil, legacy.SecurityLevelMedium, nil)
				assert.Error(t, err)
			})
			...
			t.Run("returns error for invalid trits", func(t *testing.T) {
				...
				_, err := key.Sponge(trits, legacy.SecurityLevelMedium, nil)
				assert.Error(t, err)
			})

			t.Run("returns error when Kerl is used and the last trit is non-zero", func(t *testing.T) {
				trits := make(trinary.Trits, legacy.HashTrinarySize)
				trits[legacy.HashTrinarySize-1] = 1

				_, err := key.Sponge(trits, legacy.SecurityLevelMedium, kerl.NewKerl())
				assert.Error(t, err)
			})
		})

		t.Run("SpongeFunction is reset", func(t *testing.T) {
			h := kerl.NewKerl()
			_, err := key.Sponge(nullHashTrits, legacy.SecurityLevelMedium, h)
			assert.NoError(t, err)
			// absorb is not possible after a squeeze
			assert.NoError(t, h.Absorb(nullHashTrits))
			// compare against new Kerl instance
			newKerl := kerl.NewKerl()
			_ = newKerl.Absorb(nullHashTrits)
			assert.Equal(t, newKerl.MustSqueeze(legacy.HashTrinarySize), h.MustSqueeze(legacy.HashTrinarySize))
		})
	})
...
func ExampleSponge() {
	subseed := "CEFLDDLMF9TO9ZLLTYXIPVFIJKAOFRIQLGNYIDZCTDYSWMNXPYNGFAKHQDY9ABGGQZHEFTXKWKWZXEIUD"
	entropy := trinary.MustTrytesToTrits(subseed)
	privKey, err := key.Sponge(entropy, legacy.SecurityLevelMedium, kerl.NewKerl())
	if err != nil {
		// handle error
		return
	}
