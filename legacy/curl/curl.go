...
const (
	...
	StateSize = legacy.HashTrinarySize * 3
...
func (c *Curl) Squeeze(length int) (Trits, error) {
	...
	for i := 1; i <= length/legacy.HashTrinarySize; i++ {
		copy(out[legacy.HashTrinarySize*(i-1):legacy.HashTrinarySize*i], c.State[:legacy.HashTrinarySize])
		c.Transform()
	}
...
func (c *Curl) Absorb(in Trits) error {
	...
	for i := 0; i < len(in); i += lenn {
		lenn = legacy.HashTrinarySize

		if len(in)-i < legacy.HashTrinarySize {
			lenn = len(in) - i
		}
...
func HashTrits(trits Trits, rounds ...CurlRounds) (Trits, error) {
	c := NewCurl(rounds...)
	c.Absorb(trits)
	return c.Squeeze(legacy.HashTrinarySize)
}
...
func HashTrytes(t Trytes, rounds ...CurlRounds) (Trytes, error) {
	...
	return c.SqueezeTrytes(legacy.HashTrinarySize)
}
