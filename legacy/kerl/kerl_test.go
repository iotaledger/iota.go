...
func TestHashValidTrits(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			trits, err := k.Squeeze(len(test.expected) * legacy.HashTrinarySize / legacy.HashTrytesSize)
...
func TestHashValidTrytes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			trytes, err := k.SqueezeTrytes(len(test.expected) * legacy.HashTrinarySize / legacy.HashTrytesSize)
			assert.NoError(t, err)
...
func TestErrorForWriteAfterRead(t *testing.T) {
	k := kerl.NewKerl()
	written, err := k.Write(make([]byte, legacy.HashBytesSize))
	assert.NoError(t, err)
	assert.Equal(t, legacy.HashBytesSize, written)
	read, err := k.Read(make([]byte, legacy.HashBytesSize))
	assert.NoError(t, err)
	assert.Equal(t, legacy.HashBytesSize, read)
	// write again
	written, err = k.Write(make([]byte, legacy.HashBytesSize))
...
func TestHashInvalidTrits(t *testing.T) {
	...
	t.Run("should return an error for Absorb after Squeeze", func(t *testing.T) {
		k := kerl.NewKerl()
		assert.NoError(t, k.Absorb(make(trinary.Trits, legacy.HashTrinarySize)))
		_, err := k.Squeeze(legacy.HashTrinarySize)
		assert.NoError(t, err)
		// absorb again
		assert.Equal(t, kerl.ErrAbsorbAfterSqueeze, k.Absorb(make(trinary.Trits, legacy.HashTrinarySize)))
	})
}
...
func TestHashInvalidTrytes(t *testing.T) {
	...
	t.Run("should return an error for Absorb after Squeeze", func(t *testing.T) {
		k := kerl.NewKerl()
		assert.NoError(t, k.AbsorbTrytes(legacy.NullHashTrytes))
		_, err := k.SqueezeTrytes(legacy.HashTrinarySize)
		assert.NoError(t, err)
		// absorb again
		assert.Equal(t, kerl.ErrAbsorbAfterSqueeze, k.AbsorbTrytes(legacy.NullHashTrytes))
	})
}
...
func TestKerlReset(t *testing.T) {
	t.Run("reset during absorb", func(t *testing.T) {
		k1 := kerl.NewKerl()
		assert.NoError(t, k1.Absorb(make(trinary.Trits, legacy.HashTrinarySize)))

		k1.Reset()
		k2 := kerl.NewKerl()
		assert.Equal(t, k1.MustSqueeze(legacy.HashTrinarySize), k2.MustSqueeze(legacy.HashTrinarySize))
	})

	t.Run("reset during squeeze", func(t *testing.T) {
		k1 := kerl.NewKerl()
		assert.NoError(t, k1.Absorb(make(trinary.Trits, legacy.HashTrinarySize)))
		k1.MustSqueeze(legacy.HashTrinarySize)
...
func TestKerlClone(t *testing.T) {
	t.Run("clone during absorb", func(t *testing.T) {
		...
		assert.NoError(t, k1.Absorb(make(trinary.Trits, legacy.HashTrinarySize)))
		assert.NoError(t, k2.Absorb(make(trinary.Trits, legacy.HashTrinarySize)))

		k1Clone := k1.Clone()
		assert.NoError(t, k1.Absorb(make(trinary.Trits, legacy.HashTrinarySize)))

		hash1 := k1.MustSqueeze(legacy.HashTrinarySize)
		hash2 := k2.MustSqueeze(legacy.HashTrinarySize)
		hash1Clone := k1Clone.MustSqueeze(legacy.HashTrinarySize)
	...
	t.Run("clone during squeeze", func(t *testing.T) {
		...
		k1.MustSqueeze(legacy.HashTrinarySize)
		k2.MustSqueeze(legacy.HashTrinarySize)
