...
func (b *ProtocolBlock) syntacticallyValidate() error {
	...
	if commitmentSlot > 0 && // Don't filter commitments to genesis based on being too recent.
		...
		return ierrors.Wrapf(ErrCommitmentTooRecent, "block at slot %d committing to slot %d", blockSlot, b.SlotCommitmentID.Index())
	}

	// Check that commitment is not too old.
	if blockSlot > commitmentSlot+maxCommittableAge {
		return ierrors.Wrapf(ErrCommitmentTooOld, "block at slot %d committing to slot %d, max committable age %d", blockSlot, b.SlotCommitmentID.Index(), maxCommittableAge)
	}
...
func (b *BasicBlock) syntacticallyValidate(protocolBlock *ProtocolBlock) error {
	if b.Payload != nil && b.Payload.PayloadType() == PayloadTransaction {
		...
		if cInput := tx.CommitmentInput(); cInput != nil {
			cInputSlot := cInput.CommitmentID.Index()
			...
			if cInputSlot > 0 && // Don't filter commitments to genesis based on being too recent.
				...
				return ierrors.Wrapf(ErrCommitmentInputTooRecent, "block at slot %d with commitment input to slot %d", blockSlot, cInput.CommitmentID.Index())
			}
			// Check that commitment input is not too old.
			if blockSlot > cInputSlot+maxCommittableAge {
				return ierrors.Wrapf(ErrCommitmentInputTooOld, "block at slot %d committing to slot %d, max committable age %d", blockSlot, cInput.CommitmentID.Index(), maxCommittableAge)
			}

			if cInputSlot > protocolBlock.SlotCommitmentID.Index() {
