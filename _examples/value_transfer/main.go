...
func main() {
	...
	// use ToAddress() to validate the address
	// if the address contains a checksum it will be thrown away
	targetAddr, err := signing.ToAddress(trinary.Trytes(recipientAddress))
	must(err)
	...
	bundle, err := api.PrepareTransfers(seed, transfers, inputs, unusedAddr, 2)
	must(err)
	...
	_, err = api.SendTrytes(3, bundle, 14, powFunc)
	must(err)
