...
type SendOracle interface {
	...
	// under the given conditional deposit request parameters.
	OkToSend(conds *deposit.Conditions) (bool, string, error)
}

// OracleSource defines an input for the SendOracle.
type OracleSource interface {
	Ok(conds *deposit.Conditions) (bool, string, error)
}
...
func (so *sendoracle) OkToSend(conds *deposit.Conditions) (bool, string, error) {
