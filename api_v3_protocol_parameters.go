...
func WithRewardsOptions(validatorBlocksPerSlot, profitMarginExponent, decayBalancingConstantExponent, poolCoefficientExponent uint8, bootstrappingDuration EpochIndex, rewardsManaShareCoefficient, decayBalancingConstant uint64) options.Option[V3ProtocolParameters] {
	return func(p *V3ProtocolParameters) {
		...
		result, err = safemath.SafeMul(result, uint64(p.basicProtocolParameters.ManaStructure.ManaGenerationRate))
		if err != nil {
			panic(ierrors.Wrap(err, "failed to calculate target reward due to multiplication with generationRate overflow"))
		}

		subExponent, err := safemath.SafeSub(p.basicProtocolParameters.ManaStructure.ManaGenerationRateExponent, p.basicProtocolParameters.SlotsPerEpochExponent)
		if err != nil {
