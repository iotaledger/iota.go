...
func (p *V3ProtocolParameters) String() string {
	return fmt.Sprintf("ProtocolParameters: {\n\tVersion: %d\n\tNetwork Name: %s\n\tBech32 HRP Prefix: %s\n\tRent Structure: %v\n\tToken Supply: %d\n\tGenesis Unix Timestamp: %d\n\tSlot Duration in Seconds: %d\n\tSlots per Epoch Exponent: %d\n\tMana Generation Rate: %d\n\tMana Generation Rate Exponent: %d\t\nMana Decay Factors: %v\n\tMana Decay Factors Exponent: %d\n\tMana Decay Factor Epochs Sum: %d\n\tMana Decay Factor Epochs Sum Exponent: %d\n\tStaking Unbonding Period: %d\n\tEviction Age: %d\n\tLiveness Threshold: %d\n}",
		p.defaultProtocolParameters.Version, p.defaultProtocolParameters.NetworkName, p.defaultProtocolParameters.Bech32HRP, p.defaultProtocolParameters.RentStructure, p.defaultProtocolParameters.TokenSupply, p.defaultProtocolParameters.GenesisUnixTimestamp, p.defaultProtocolParameters.SlotDurationInSeconds, p.defaultProtocolParameters.SlotsPerEpochExponent, p.defaultProtocolParameters.ManaGenerationRate, p.defaultProtocolParameters.ManaGenerationRateExponent, p.defaultProtocolParameters.ManaDecayFactors, p.defaultProtocolParameters.ManaDecayFactorsExponent, p.defaultProtocolParameters.ManaDecayFactorEpochsSum, p.defaultProtocolParameters.ManaDecayFactorEpochsSumExponent, p.defaultProtocolParameters.StakingUnbondingPeriod, p.defaultProtocolParameters.EvictionAge, p.defaultProtocolParameters.LivenessThreshold)
}
