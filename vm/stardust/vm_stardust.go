...
func accountGenesisValid(current *iotago.AccountOutput, vmParams *vm.Params) error {
	...
	if nextBIFeat := current.FeatureSet().BlockIssuer(); nextBIFeat != nil && nextBIFeat.ExpirySlot != 0 && nextBIFeat.ExpirySlot < vmParams.WorkingSet.Tx.Essence.CreationTime+vmParams.External.ProtocolParameters.AllowedCommitmentsWindowSize {
...
func accountBlockIssuerSTVF(input *vm.ChainOutputWithCreationTime, next *iotago.AccountOutput, vmParams *vm.Params) error {
	...
	if currentBIFeat.ExpirySlot >= txSlotIndex {
		...
		if nextBIFeat.ExpirySlot != 0 && nextBIFeat.ExpirySlot != currentBIFeat.ExpirySlot && nextBIFeat.ExpirySlot < txSlotIndex+vmParams.External.ProtocolParameters.AllowedCommitmentsWindowSize {
			return fmt.Errorf("%w: block issuer feature expiry set too soon", iotago.ErrInvalidBlockIssuerTransition)
		}

	} else if nextBIFeat != nil {
		// if the block issuer feature has expired, it must either be removed or expiry extended.
		if nextBIFeat.ExpirySlot != 0 && nextBIFeat.ExpirySlot < txSlotIndex+vmParams.External.ProtocolParameters.AllowedCommitmentsWindowSize {
			return fmt.Errorf("%w: block issuer feature expiry set too soon", iotago.ErrInvalidBlockIssuerTransition)
		}
	}
	...
	for _, output := range vmParams.WorkingSet.OutputsByType[iotago.OutputBasic] {
		...
		if basicOutput.UnlockConditionSet().HasManalockCondition(current.AccountID, txSlotIndex+vmParams.External.ProtocolParameters.AllowedCommitmentsWindowSize) {
	...
	for _, output := range vmParams.WorkingSet.OutputsByType[iotago.OutputNFT] {
		...
		if nftOutput.UnlockConditionSet().HasManalockCondition(current.AccountID, txSlotIndex+vmParams.External.ProtocolParameters.AllowedCommitmentsWindowSize) {
