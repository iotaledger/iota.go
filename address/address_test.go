...
func TestGenerateAddress(t *testing.T) {
	...
	for i := 0; i < len(addresses); i++ {
		addr, err := address.GenerateAddress(seed, uint64(i), consts.SecurityLevelMedium)
		assert.NoError(t, err)
		assert.Equal(t, addresses[i], addr)
	}

	for i := 0; i < len(addresses); i++ {
		addr, err := address.GenerateAddress(seed, uint64(i), consts.SecurityLevelMedium, true)
		assert.NoError(t, err)
...
func TestGenerateAddresses(t *testing.T) {
	addrs, err := address.GenerateAddresses(seed, 0, 5, consts.SecurityLevelMedium, false)
	assert.NoError(t, err)
	assert.Equal(t, addresses, addrs)

	addrs, err = address.GenerateAddresses(seed, 0, 5, consts.SecurityLevelMedium, true)
	assert.NoError(t, err)
