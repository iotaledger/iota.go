...
func (blsAddr *BLSAddress) Size() int {
	return BLSAddressSerializedBytesSize
}

func (blsAddr *BLSAddress) Clone() Address {
	cpy := &BLSAddress{}
	copy(cpy[:], blsAddr[:])
	return cpy
}
...
func (blsAddr *BLSAddress) VByteCost(costStruct *RentStructure, _ VByteCostFunc) uint64 {
	return costStruct.VBFactorData.Multiply(serializer.SmallTypeDenotationByteSize + BLSAddressBytesLength)
}

func (blsAddr *BLSAddress) Key() string {
	return string(append([]byte{byte(AddressBLS)}, (*blsAddr)[:]...))
}

func (blsAddr *BLSAddress) Unlock(msg []byte, sig Signature) error {
	blsSig, isBLSSig := sig.(*BLSSignature)
	if !isBLSSig {
		return fmt.Errorf("%w: can not unlock BLS address with signature of type %s", ErrSignatureAndAddrIncompatible, sig)
	}
...
func (blsAddr *BLSAddress) Deserialize(data []byte, deSeriMode serializer.DeSerializationMode, deSeriCtx interface{}) (int, error) {
	if deSeriMode.HasMode(serializer.DeSeriModePerformValidation) {
		...
		if err := serializer.CheckTypeByte(data, byte(AddressBLS)); err != nil {
...
func (blsAddr *BLSAddress) MarshalJSON() ([]byte, error) {
	...
	jBLSAddress.Address = EncodeHex(blsAddr[:])
	jBLSAddress.Type = int(AddressBLS)
...
func (j *jsonBLSAddress) ToSerializable() (serializer.Serializable, error) {
	addrBytes, err := DecodeHex(j.Address)
	if err != nil {
