package key_test

import (
	"encoding/hex"
	"encoding/json"
	"fmt"
	"os"
	"path/filepath"
	"testing"

	"github.com/iotaledger/iota.go/consts"
	"github.com/iotaledger/iota.go/kerl"
	"github.com/iotaledger/iota.go/signing/key"
	"github.com/iotaledger/iota.go/trinary"
	"github.com/stretchr/testify/assert"
)

var nullHashTrits = make(trinary.Trits, consts.HashTrinarySize)

func ReadTestJSONFile(t *testing.T, name string, v interface{}) {
	path := filepath.Join("testdata", name)
	f, err := os.Open(path)
	if err != nil {
		t.Fatal(err)
	}
	defer f.Close()

	dec := json.NewDecoder(f)
	if err := dec.Decode(v); err != nil {
		t.Fatal(err)
	}
}

func TestShake(t *testing.T) {
	t.Run("null entropy", func(t *testing.T) {
		var (
			// compare against official eXtended Keccak Code Package https://github.com/XKCP/XKCP
			// KeccakSum --shake256 --hex --outputbits 31104 <(printf '\x00%.0s' {1..48})
			shakeHex = ""
			// the SHAKE256 sum converted to trits
			shakeTrits trinary.Trits
		)

		shakeSum, _ := hex.DecodeString(shakeHex)
		for i := 0; i < len(shakeSum); i += consts.HashBytesSize {
			trits, _ := kerl.KerlBytesToTrits(shakeSum[i : i+consts.HashBytesSize])
			shakeTrits = append(shakeTrits, trits...)
		}

		tests := []struct {
			name   string
			secLvl consts.SecurityLevel
		}{
			{name: "SecurityLevelLow", secLvl: consts.SecurityLevelLow},
			{name: "SecurityLevelMedium", secLvl: consts.SecurityLevelMedium},
			{name: "SecurityLevelHigh", secLvl: consts.SecurityLevelHigh},
		}

		for _, test := range tests {
			t.Run(test.name, func(t *testing.T) {
				k, err := key.Shake(nullHashTrits, test.secLvl)
				assert.NoError(t, err)
				assert.Len(t, k, int(test.secLvl)*consts.SignatureMessageFragmentTrinarySize)
				assert.Equal(t, k, shakeTrits[:len(k)])
			})
		}
	})

	t.Run("valid entropy", func(t *testing.T) {
		t.Run("testdata/shake.json", func(t *testing.T) {
			var tests []*struct {
				Entropy       trinary.Trytes               `json:"entropy"`
				SecurityLevel consts.SecurityLevel `json:"securityLevel"`
				Key           trinary.Trytes               `json:"key"`
			}
			ReadTestJSONFile(t, "shake.json", &tests)

			for _, tt := range tests {
				t.Run(tt.Entropy, func(t *testing.T) {
					k, err := key.Shake(trinary.MustTrytesToTrits(tt.Entropy), tt.SecurityLevel)
					assert.NoError(t, err)
					assert.Equal(t, tt.Key, trinary.MustTritsToTrytes(k))
				})
			}
		})
	})

	t.Run("invalid entropy", func(t *testing.T) {
		t.Run("returns error for empty entropy", func(t *testing.T) {
			_, err := key.Shake(nil, consts.SecurityLevelMedium)
			assert.Error(t, err)
		})

		t.Run("returns error for invalid trits ", func(t *testing.T) {
			trits := make(trinary.Trits, consts.HashTrinarySize)
			trits[0] = consts.MaxTritValue + 1
			_, err := key.Shake(trits, consts.SecurityLevelMedium)
			assert.Error(t, err)
		})

		t.Run("returns error when last trit is non-zero", func(t *testing.T) {
			trits := make(trinary.Trits, consts.HashTrinarySize)
			trits[consts.HashTrinarySize-1] = 1
			_, err := key.Shake(trits, consts.SecurityLevelMedium)
			assert.Error(t, err)
		})
	})
}

func ExampleShake() {
	subseed := "CEFLDDLMF9TO9ZLLTYXIPVFIJKAOFRIQLGNYIDZCTDYSWMNXPYNGFAKHQDY9ABGGQZHEFTXKWKWZXEIUD"
	entropy := trinary.MustTrytesToTrits(subseed)
	privKey, err := key.Shake(entropy, consts.SecurityLevelMedium)
	if err != nil {
		// handle error
		return
	}
	fmt.Println(trinary.MustTritsToTrytes(privKey))
	// Output: 
}

func TestSponge(t *testing.T) {

	t.Run("Sponge()", func(t *testing.T) {

		t.Run("Kerl", func(t *testing.T) {
			kerlTrytes := ""

			tests := []struct {
				name   string
				secLvl consts.SecurityLevel
			}{
				{name: "SecurityLevelLow", secLvl: consts.SecurityLevelLow},
				{name: "SecurityLevelMedium", secLvl: consts.SecurityLevelMedium},
				{name: "SecurityLevelHigh", secLvl: consts.SecurityLevelHigh},
			}

			for _, test := range tests {
				t.Run(test.name, func(t *testing.T) {
					sponge := kerl.NewKerl()
					key, err := key.Sponge(nullHashTrits, test.secLvl, sponge)
					assert.NoError(t, err)
					assert.Len(t, trinary.MustTritsToTrytes(key), int(test.secLvl)*consts.SignatureMessageFragmentSizeInTrytes)
					assert.Equal(t, trinary.MustTritsToTrytes(key), kerlTrytes[:len(key)/consts.TritsPerTryte])
				})
			}
		})

		t.Run("invalid entropy", func(t *testing.T) {
			t.Run("returns error for empty entropy", func(t *testing.T) {
				_, err := key.Sponge(nil, consts.SecurityLevelMedium, nil)
				assert.Error(t, err)
			})

			t.Run("returns error for invalid trits", func(t *testing.T) {
				trits := make(trinary.Trits, consts.HashTrinarySize)
				trits[0] = consts.MaxTritValue + 1

				_, err := key.Sponge(trits, consts.SecurityLevelMedium, nil)
				assert.Error(t, err)
			})

			t.Run("returns error when Kerl is used and the last trit is non-zero", func(t *testing.T) {
				trits := make(trinary.Trits, consts.HashTrinarySize)
				trits[consts.HashTrinarySize-1] = 1

				_, err := key.Sponge(trits, consts.SecurityLevelMedium, kerl.NewKerl())
				assert.Error(t, err)
			})
		})

		t.Run("SpongeFunction is reset", func(t *testing.T) {
			h := kerl.NewKerl()
			_, err := key.Sponge(nullHashTrits, consts.SecurityLevelMedium, h)
			assert.NoError(t, err)
			// absorb is not possible after a squeeze
			assert.NoError(t, h.Absorb(nullHashTrits))
			// compare against new Kerl instance
			newKerl := kerl.NewKerl()
			_ = newKerl.Absorb(nullHashTrits)
			assert.Equal(t, newKerl.MustSqueeze(consts.HashTrinarySize), h.MustSqueeze(consts.HashTrinarySize))
		})
	})

}

func ExampleSponge() {
	subseed := "CEFLDDLMF9TO9ZLLTYXIPVFIJKAOFRIQLGNYIDZCTDYSWMNXPYNGFAKHQDY9ABGGQZHEFTXKWKWZXEIUD"
	entropy := trinary.MustTrytesToTrits(subseed)
	privKey, err := key.Sponge(entropy, consts.SecurityLevelMedium, kerl.NewKerl())
	if err != nil {
		// handle error
		return
	}
	fmt.Println(trinary.MustTritsToTrytes(privKey))
	// output: 
}
