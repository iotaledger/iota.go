...
// Interface for Construction of Iota Hash Functions
type IotaHashFunc interface {
	New() IotaHash
}

// Interface for Iota Hash Functions
...
// Use this struct to hash with Curl
type CurlHash struct{}

// Use this struct to hash with Kerl
...
// Checks if a hash function was given, otherwise uses Kerl
...
// Optional takes the hashFunc to use (e.g. &KerlHash{}, &CurlHash{})
...
func ValidateSignatures(expectedAddress Hash, fragments []Trytes, bundleHash Hash, hashFunc ...IotaHashFunc) (bool, error) {
	...
	for i := 0; i < len(fragments); i++ {
		digest, err := Digest(normalizedBundleHashFragments[i%MaxSecurityLevel], MustTrytesToTrits(fragments[i]), hashCon)
		if err != nil {
			return false, err
		}
		for j := 0; j < HashTrinarySize; j++ {
